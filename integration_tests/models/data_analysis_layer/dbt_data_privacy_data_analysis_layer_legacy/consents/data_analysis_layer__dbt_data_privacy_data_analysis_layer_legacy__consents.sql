...
WITH privacy_protected_model AS (
  SELECT
    ...
    SHA256(CAST(pseudonymized_user_id AS STRING)) AS `pseudonymized_user_id`,
    STRUCT(
  ...
  FROM
    {{ ref('restricted_layer__dbt_data_privacy_restricted_layer_legacy__consents') }} AS __original_table
  WHERE
